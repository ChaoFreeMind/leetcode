...
public class Solution {
    ...
    // If there are one number or two numbers left and it's player1's turn, player1 will always win
    public boolean PredictTheWinner(int[] nums) {
        if (nums == null || nums.length == 0) return false;
        return predictFirst(nums, 0, nums.length - 1, 0, 0);
    }

    // Predict if first player can win.
    private boolean predictFirst(int[] nums, int left, int right, int score1, int score2) {
        if (left == right) {
            return score1 + nums[left] >= score2;
        }
        // If current player choose left or right and as a result the second player lost, then current player win
        return !predictSecond(nums, left + 1, right, score1 + nums[left], score2)
            || !predictSecond(nums, left, right - 1, score1 + nums[right], score2);
    }

    // Predict if second player can win.
    private boolean predictSecond(int[] nums, int left, int right, int score1, int score2) {
        if (left == right) {
            return score2 + nums[left] > score1; // Player2 can only win if his score is greater than player1
        }
        // If current player choose left or right and as a result the second player lost, then current player win
        return !predictFirst(nums, left + 1, right, score1, score2 + nums[left])
